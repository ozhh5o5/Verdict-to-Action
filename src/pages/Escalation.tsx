...
import { ArrowUpRight, Scale as ScaleIcon, User, FileText, Calendar, ChevronRight } from 'lucide-react'
...
export default function Escalation() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setEvents(MOCK_DATA.escalations as any || [])
      setLoading(false)
    }, 500)
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-8">
              ...
              <div className="hidden md:flex items-center gap-1">
                ...
                <Link to="/escalation" className="px-3 py-2 rounded-md text-sm font-medium text-white bg-slate-800 transition-colors">Escalations</Link>
              </div>
            </div>
          </div>
        </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <Link to={`/obligations/${oblId}`} className="hover:underline">
                        <h3 className="text-base font-semibold text-white">{obl.title}</h3>
                      </Link>
                      <div className="flex items-center gap-3 text-sm text-slate-500 mt-1">
                        <Link to={`/judgments/${obl.judgment.id}`} className="hover:text-slate-300 flex items-center gap-1">
