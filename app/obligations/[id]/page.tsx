...
import { CheckCircle2, Calendar, User, FileText, AlertTriangle, Lightbulb, ChevronRight, ArrowUpRight, Scale } from 'lucide-react'
...
export default function ObligationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleEscalate = async () => {
    ...
    try {
      await fetch(`/api/obligations/${id}/escalate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ triggeredBy: 'Manual Escalation by Officer' })
      })
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <nav className="border-b border-slate-800 bg-slate-900/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <Link href="/obligations" className="text-sm text-slate-400 hover:text-white mb-2 inline-flex items-center gap-1">
            ← Back to Obligations
          </Link>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 text-sm text-slate-400 flex-wrap">
                <Link href={`/judgments/${obligation.judgment.id}`} className="hover:text-blue-400 flex items-center gap-1 transition-colors">
