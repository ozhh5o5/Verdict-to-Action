...
import { FileText, Scale, AlertCircle, CheckCircle2, Upload, ShieldAlert, ArrowUpRight, BookOpen, AlertTriangle } from 'lucide-react'
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/dashboard/escalations').then(r => r.json()),
    ]).then(([statsData, deadlinesData, riskData, escData]) => {
      setStats(statsData)
      setDeadlines(deadlinesData.deadlines || [])
      setRisks(riskData.risks || [])
      setEscalationCount(escData.events?.length || 0)
      setLoading(false)
    })
  }, [])
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-8">
              ...
              <div className="hidden md:flex items-center gap-1">
                <Link href="/judgments" className="px-3 py-2 rounded-md text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                  Judgments
                </Link>
                <Link href="/obligations" className="px-3 py-2 rounded-md text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                  Obligations
                </Link>
                <Link href="/contempt-risk" className="px-3 py-2 rounded-md text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                  Contempt Risk
                </Link>
                <Link href="/escalation" className="px-3 py-2 rounded-md text-sm font-medium text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
                  Escalations
                </Link>
              </div>
            </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {risks.slice(0, 3).map(risk => (
                  <Link key={risk.judgmentId} href={`/judgments/${risk.judgmentId}`}>
